...
void framecut(AVFrame * oframe , AVFrame * iframe, int _w_min, int _h_min, int _w_max, int _h_max, AVPixelFormat fmt)
{
	...
	int h[3], w[3], h_offset[3], w_offset[3], j1;
	unsigned char* pnt;
      	switch (fmt) {
		...
		//warning: for yuvj422p, output a yuv420p frame, to make the frame acceptable for nvenc codec
                case AVPixelFormat::AV_PIX_FMT_YUVJ422P:
                        ...
                        pnt = oframe->data[0];
                        for (int j = 0; j < h[0]; j++) {
				if (upside_down) j1 = h[0] - j - 1; else j1 = j;
                        	memcpy(pnt + oframe->linesize[0] * j1 , iframe->data[0] + iframe->linesize[0] * (j+ h_offset[0]) + w_offset[0], w[0] * sizeof(char));
			}
			for (int i = 1; i < 3; ++i) {
                                pnt = oframe->data[i];
                                for (int j = 0; j < h[i]; j+=2) {
					if (upside_down) j1 = (((h[i] - 1) >> 1) << 1) - j; else j1 = j;
                                        memcpy(pnt + oframe->linesize[i] * (j1 >> 1) , iframe->data[i] + iframe->linesize[i] * (j+ h_offset[i]) + w_offset[i], w[i] * sizeof(char));
				}
...
{
	int w_min = ((int)((1.0 - framecut_ratio) / 2 * input_w) >> 1) << 1;
	int w_max = (((int)((1.0 + framecut_ratio) / 2 * input_w) >> 1) << 1) - 1;
	int h_pixels = (((w_max - w_min + 1) * 5 / 9) >> 1) << 1;
	int h_min = (input_h - h_pixels) >> 1;
	int h_max = ((input_h + h_pixels) >> 1) - 1;
	...
	string input_stream;
	if (thread_id < 4) input_stream = "http://192.168.88.102:8000/stream"; else input_stream = "http://192.168.88.103:8000/stream";
	//string input_stream = "http://127.0.0.1:8000/stream";
	input_stream[25] += thread_id;
	...
	codec_ctx_2h264 = ostream_video->codec;
        ...
        codec_ctx_2h264->bit_rate = 100000000;
			...
			while (1) {
				...
				if ((video_count & 3) == 0) {
					if (thread_sum > 1) {
						...
						if ((video_count & 255) == 0) printf("Thread %d:  frame %d\n", thread_id, video_count);
					}
				}
			}
		}
